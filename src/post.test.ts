...
describe("post — --inline-template", () => {
  ...
  it("uses the bundled inline.eta template — with its [!TIP] disclosure — when --inline-template is omitted (issue #22 regression)", async () => {
    ...
    // title + severity/confidence info line (issues #12, #27) AND the [!TIP] disclosure fold
    // (issue #16/#22), so the disclosure is no longer reachable only via an explicit
    // --inline-template.
    expect(commentBody).toContain("**Test finding**");
    expect(commentBody).toContain("🔵 · 0.70 confidence");
