...
An adapter maps one agent CLI's native result envelope onto the **abstract result envelope** (the
deliverable of [SPEC §3.2](../SPEC.md#32-the-deliverable)) and does nothing else. Its inputs, outputs,
and prohibitions are normative in SPEC — this file explains the shape and the reference invocation;
[`src/schema.ts`](../src/schema.ts) owns the field lists so they can't rot out of sync here.

### Input

The adapter receives the reviewer's inputs — the PR diff, the CI result, prior-review context, an
optional test report, and failing-job logs — as **files** (SPEC §2.2, §3.1). It never fetches them
itself (no network) and never takes untrusted text as a shell argument or environment variable
(SPEC §4.4). The CI result routes the run per [SPEC §3.1](../SPEC.md#31-trigger--routing): pass →
full review, fail → mechanic-only pass, cancelled/skipped/not-run → post nothing.

### Output

The adapter emits the abstract result envelope with the field names owned by
[`src/schema.ts`](../src/schema.ts) — `schema_version`, `findings`, `models`, `turns`,
...
presentation and posting are the commenter's job (SPEC §2.3; conformance in
[SPEC §5](../SPEC.md#5-conformance)).
...
[`examples/workflows/review.yaml`](../examples/workflows/review.yaml) — the spec leaves this to the
reference implementation (SPEC Appendix A). Note the auth variable is `ANTHROPIC_AUTH_TOKEN`, not
...
"last JSON wins" parser would obey ([SPEC §4.2](../SPEC.md#42-threats)). The residual
total-replacement injection — one forged survivor left after the genuine block is suppressed entirely
— is out of the ladder's scope and is contained by the §4.2 controls (read-only token, egress lock,
...
exits 0 but synthesizes `safe: false` ([SPEC §4.3](../SPEC.md#43-preflight-triage)); only
findings extraction exits non-zero on recovery failure.

Caveat: a finding `description` that itself contains a complete, valid findings-JSON example (e.g. as
...
as files, emit the abstract envelope (the deliverable of SPEC §3.2) mapped from the CLI's native
output, recover `findings` via the same extraction ladder when structured-output enforcement is
imperfect, never post to GitHub, and run under the same controls (read-only token, egress lock,
burner key with spend cap). The full conformance requirements are the Reviewer bullet in
[SPEC §5](../SPEC.md#5-conformance); the reference to mirror is [`src/adapt.ts`](../src/adapt.ts).
